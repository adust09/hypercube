...
fn test_tsl_encoding_basic() {
    ...
    let encoded = tsl.encode(message, randomness);
    
    // Verify the encoded vertex is in the correct layer
    let layer = hypercube_signatures::core::hypercube::Hypercube::new(4, 4)
        .calculate_layer(&encoded);
...
fn test_tsl_encoding_deterministic() {
    ...
    let encoded1 = tsl.encode(message, randomness);
    let encoded2 = tsl.encode(message, randomness);
...
fn test_tsl_encoding_different_messages() {
    ...
    let encoded1 = tsl.encode(b"message1", randomness);
    let encoded2 = tsl.encode(b"message2", randomness);
...
fn test_tsl_non_uniform_mapping() {
    ...
    for i in 0..100 {
        let vertex = tsl.map_to_layer(i);
        let layer = hypercube_signatures::core::hypercube::Hypercube::new(4, 4)
            .calculate_layer(&vertex);
        assert_eq!(layer, 4);
    }
}
...
fn test_tsl_uniform_distribution() {
    ...
    for i in 0..num_samples {
        ...
        let idx = hypercube_signatures::core::mapping::vertex_to_integer(
            vertex.components(), 3, 3, 3
        );
        counts[idx] += 1;
    }
