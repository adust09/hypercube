use hypercube_signatures::xmss::{XMSSParams, XMSSKeypair};
...
fn main() {
    ...
    if is_valid {
        ...
        println!("  Debug: PK root length = {}", keypair.public_key().root().len());
        println!("  Debug: Auth path has {} nodes", signature.auth_path().nodes().len());
    }
