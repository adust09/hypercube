...
use hypercube_signatures::core::mapping::{vertex_to_integer, integer_to_vertex, NonUniformMappingPsi};
...
fn main() {
    ...
    match NonUniformMappingPsi::new(w, v, d) {
        Ok(psi) => {
            ...
            for value in 0..5 {
                match psi.map(value) {
                    Ok(vertex) => {
                        match psi.probability(&vertex) {
                            Ok(prob) => {
                                println!("  Value {} → {:?} (prob: {:.4})", value, vertex, prob);
                            }
                            Err(e) => println!("  Value {} → {:?} (prob error: {:?})", value, vertex, e),
                        }
                    }
    ...
    match NonUniformMappingPsi::new(w2, v2, d2) {
        Ok(psi) => {
            println!("Layer size for [{}]^{}, layer {}: {}", w2, v2, d2, psi.layer_size());
