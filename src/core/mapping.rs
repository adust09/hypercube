...
use num_traits::{Zero, One, ToPrimitive, CheckedSub};

/// Maps a vertex in layer d to an integer in [0, ℓ_d)
/// Implements the exact mapping from the paper Section 6.1
pub fn vertex_to_integer(vertex: &[usize], w: usize, v: usize, d: usize) -> Result<usize, MappingError> {
...
fn vertex_to_integer_paper_exact(vertex: &[usize], w: usize, v: usize, d: usize) -> Result<usize, MappingError> {
    ...
    for (_pos, &coord) in vertex.iter().enumerate() {
        ...
        if remaining_sum < used_sum {
            return Err(MappingError::InvalidLayer { expected: d, actual: v * w - vertex.iter().sum::<usize>() });
        }
...
pub fn integer_to_vertex(i: usize, w: usize, v: usize, d: usize) -> Result<Vec<usize>, MappingError> {
...
fn integer_to_vertex_paper_exact(mut index: usize, w: usize, v: usize, d: usize) -> Result<Vec<usize>, MappingError> {
    ...
    for pos in 0..v {
        ...
        for coord in 1..=w {
            ...
            if remaining_sum >= used_sum {
                ...
                let completions = completions_big.to_usize().ok_or(MappingError::IntegerOverflow)?;
