...
//
// Paper: "At the Top of the Hypercube"
...
mod tests {
    ...
    fn test_calculate_layer_size_basic() {
        ...
        assert_eq!(calculate_layer_size(0, 3, 2).unwrap().to_usize().unwrap(), 1);

        // Layer 1: vertices at distance 1 from sink
        // (1,2,2), (2,1,2), (2,2,1) -> size = 3
        assert_eq!(calculate_layer_size(1, 3, 2).unwrap().to_usize().unwrap(), 3);

        // Layer 2: vertices at distance 2 from sink
        // (1,1,2), (1,2,1), (2,1,1) -> size = 3
        assert_eq!(calculate_layer_size(2, 3, 2).unwrap().to_usize().unwrap(), 3);

        // Layer 3: only (1,1,1) -> size = 1
        assert_eq!(calculate_layer_size(3, 3, 2).unwrap().to_usize().unwrap(), 1);
    }
    ...
    fn test_calculate_layer_size_formula() {
        ...
        let size = calculate_layer_size(4, 4, 4,).unwrap().to_usize().unwrap();
        assert!(size > 0); // Exact value depends on implementation

        // Layer size should be 0 for d > v(w-1)
        assert_eq!(calculate_layer_size(10, 3, 2).unwrap().to_usize().unwrap(), 0);
        // d=10 > 3*(2-1)=3
    }
    ...
    fn test_layer_size_symmetry() {
        ...
        let mut sum = 0;
        for d in 0..=3 {
            sum += calculate_layer_size(d, 3, 2,).unwrap().to_usize().unwrap();
        }
